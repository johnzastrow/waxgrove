...
## 1. The first account is yours

Open the app. The first person to register becomes the **admin** — no invite
code needed, because there is nobody to issue one.

Everyone after that needs an invite. That is deliberate: this is a friends
instance, not a public service.

**To invite someone:** go to **You → Invites → Create an invite code**, and send
them the code. Each code works once and expires. They enter it on the register
screen along with their email, a display name and a password.
